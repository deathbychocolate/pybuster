...
from pybuster.src.constants import (BROWSER_NAME_CHROME, BROWSER_NAME_FIREFOX, BROWSER_NAME_INTERNET_EXPLORER,
                                    BROWSER_NAME_SAFARI, SUPPORTED_BROWSER_NAMES)
...
class Headers:
    ...
    def generate_random_user_agent(self):
        ...
        browser_name = random.choice(SUPPORTED_BROWSER_NAMES)
        if browser_name == BROWSER_NAME_FIREFOX:
            user_agent = self.generate_user_agent_for_firefox()
        elif browser_name == BROWSER_NAME_CHROME:
            user_agent = self.generate_user_agent_for_chrome()
        elif browser_name == BROWSER_NAME_SAFARI:
            user_agent = self.generate_user_agent_for_safari()
        elif browser_name == BROWSER_NAME_INTERNET_EXPLORER:
